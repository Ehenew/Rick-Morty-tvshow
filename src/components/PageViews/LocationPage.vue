  ...
  <div class="bg-location-cover bg-cover">
    ...
    >
      <div class="sm:col-start-2 sm:row-start-1 font-semibold text-xl">
        ...
        <p>
          Type:
          <span class="text-green-400 ml-1">{{ result.location.type }}</span>
        </p>
        <p>
          Dimension:
          <span class="text-green-400 ml-1">{{
            result.location.dimension
          }}</span>
        </p>
        <p>
          Created on:
          <span class="text-green-400 ml-1">{{
            formatDate(result.location.created)
          }}</span>
        </p>
      </div>
      ...
      >
        ...
        <div class="p-2 overflow-y-scroll max-h-80">
          <div
            ...
            :key="resident"
            class="gap-4 border-b-[1px] border-gray-600 py-1"
          >
            <router-link
      ...
      <div class="flex gap-8 row-start-1">
        <router-link to="/location">
          <button class="mt-3 px-2 py-1">&larr; Back</button>
        </router-link>
        <router-link to="/">
          <button class="mt-3 py-1 px-2">Home</button>
        </router-link>
      </div>
      ...
      >
        <a
          href="https://github.com/Ehenew/Rick-Morty.git"
          alt="Github repostitory"
          target="_blank"
        >
          <i class="lni lni-github-original hover:text-gray-400 text-2xl"></i>
        </a>
        <a
          href="https://www.figma.com/design/8Z8xR6g3MxrXWGedvVwy6y/Rick-%26-Morty---Location-Page?node-id=0-1&t=tRUPpgpFLqi2SKqH-1"
          alt="Design on Figma"
          target="_blank"
        >
          <i class="lni lni-figma hover:text-gray-400 text-2xl"></i>
        </a>
      </div>
    </div>
  </div>
